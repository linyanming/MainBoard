...
void CAN_Config(void)
{
	...
	CAN_Str.CAN_BS2 = CAN_BS2_6tq; //定义发送点位置
	CAN_Str.CAN_SJW = CAN_SJW_1tq; //
	CAN_Str.CAN_Prescaler = 6;    //时钟分频系数，CAN总线波特率=APB1时钟/((BS1+BS2+SJW)*分频系数)                 ，在这里为 500kbps
	...
	CAN_FilterStr.CAN_FilterMode = CAN_FilterMode_IdList; //标识符列表模式
	...
	NVIC_Str.NVIC_IRQChannel = CEC_CAN_IRQn;
	NVIC_Str.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Str.NVIC_IRQChannelPriority = 0;
	NVIC_Init(&NVIC_Str);
...
//	TxMsg.ExtId = 0x12;
	...
	mbox = CAN_Transmit(CAN, &TxMsg);
	switch(mbox)
	{
		case CAN_TxStatus_NoMailBox:
			while((CAN_TransmitStatus(CAN, mbox) == CAN_TxStatus_NoMailBox) && (i < 0xfff)) 
				i++;
			if(i >= 0xfff)
			{
